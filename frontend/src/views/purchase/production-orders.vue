...
function buildPoHtml(order, company, supplier) {
  ...
  return `
    ...
    <table>
      ...
      <tbody>${items.map((it, idx) => `<tr><td style="text-align:center;">${idx + 1}</td><td><strong>${it.model || ''}</strong></td><td>${it.spec_cn || it.spec || ''}<br><span style="font-size:9px; color:#64748b;">${it.name_en || ''}</span></td><td style="text-align:right; font-weight:bold;">${it.qty || 0}</td><td style="text-align:center;">${it.unit || 'PCS'}</td><td style="text-align:center;">${it.pcs_per_ctn || ''}</td><td style="text-align:center;">${it.ctns || ''}</td><td style="text-align:right;">${currSign}${formatMoney(it.price || 0)}</td></tr>`).join('')}</tbody>
